...
`.github/workflows/dependabot-lockfile.yml` closes the gap on Dependabot's
behalf: its `regenerate_python` job re-runs the two `uv pip compile`
commands above on the PR head and pushes the regenerated locks back onto the
branch, which retriggers CI. It deliberately omits `--upgrade`, so only the
pins the bump actually invalidated move and the sync commit stays scoped.
**It is inert until an operator adds the `DEPENDABOT_LOCKFILE_PAT`
Dependabot secret** — until then every Dependabot pip PR needs the locks
regenerated by hand with the commands above. See
