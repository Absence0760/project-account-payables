...
def test_vendor_spend_escapes_vendor_name_only():
    ...
    # The two trailing "" are `currencies` and `unconverted_count`: a positional
    # 3-tuple caller carries neither, so both export blank by design. The real
    # callers pass `VendorSpendEntry` rows from
    # `currency_conversion.vendor_rollup_from_grouped_rows`, which supplies both.
    assert rows[0] == [
        "vendor_name",
        "invoice_count",
        "total_amount",
