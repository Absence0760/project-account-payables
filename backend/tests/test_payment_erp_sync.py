...
async def _raise_blocking_exception(realdb, key: str, invoice_id: uuid.UUID, kind: str) -> None:
    ...
    async with mk() as s:
        s.add(
            APException(
                ...
                severity="error",
                status="open",
                description="settlement did not reconcile",
...
async def test_sync_holds_invoice_carrying_an_unresolved_blocking_exception(realdb):
    """An UNDER-settlement is the case that bites. The processor reports it
    moved half the authorized amount; the payment row is legitimately
    `completed` and `payment_webhook` raises a `fraud_flag`. Without this gate
    the invoice is still closed out as fully `paid` — which is what the ERP,
    the aging report and the 1099 YTD totals read — the vendor is shorted, and
    a `paid` invoice never re-enters a payment run to collect the rest.

    It must WAIT at `payment_scheduled` until a human works the exception
    queue (clear the flag, or void the payment and re-pay at the right
    amount)."""
    await _set_org_erp(realdb, "a", {"type": "mock", "integration_method": "direct"})
    run_id, invoice_id = await _seed_run(realdb, "a")
    await _raise_blocking_exception(realdb, "a", invoice_id, "fraud_flag")
...
async def test_sync_hold_covers_every_payment_blocking_type(realdb):
    """The gate reuses `payment_runs.blocked_invoice_ids`, so it is the same
    tuple `POST /runs` and `/retry-failed` refuse — an exception class that
    blocks money going OUT also blocks the books being closed on money that
    already went. Pinned per-type so the two rules can't drift apart."""
    from app.api.payments import PAYMENT_BLOCKING_EXCEPTION_TYPES

    await _set_org_erp(realdb, "a", {"type": "mock", "integration_method": "direct"})
    for kind in PAYMENT_BLOCKING_EXCEPTION_TYPES:
        run_id, invoice_id = await _seed_run(realdb, "a")
        await _raise_blocking_exception(realdb, "a", invoice_id, kind)
        ...
        async with mk() as s:
            inv = (await s.execute(select(Invoice).where(Invoice.id == invoice_id))).scalar_one()
        assert inv.status == InvoiceStatus.payment_scheduled, f"{kind} did not hold the invoice"


async def test_sync_pays_when_the_blocking_exception_is_resolved(realdb):
    """The other direction: a RESOLVED flag must not hold the invoice
    forever, and a non-blocking class (`po_mismatch` is advisory here) must
    not hold it at all. Otherwise the gate would strand every invoice that
    ever carried an exception."""
    await _set_org_erp(realdb, "a", {"type": "mock", "integration_method": "direct"})
    run_id, invoice_id = await _seed_run(realdb, "a")
    await _raise_blocking_exception(realdb, "a", invoice_id, "po_mismatch")
