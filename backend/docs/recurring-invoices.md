...
`generate_one` stamps `Invoice.uploaded_by_id` with
`actor_id or template.created_by_user_id` — two answers in priority order:

| Generated by | Uploader recorded |
|---|---|
| `POST /{id}/generate-now` | the caller (`actor_id`) |
| the background sweep | `RecurringInvoiceTemplate.created_by_user_id` — the employee who authored the template |
| the sweep, on a template created before migration 0096 | NULL (legacy) |

That column is what segregation of duties keys on
