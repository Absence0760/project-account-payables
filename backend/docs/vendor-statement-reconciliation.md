...
| `GET /vendor-statements/summary` | Whole-set KPI rollup — `by_status` counts + `open_discrepancies` (SUM of `amount_mismatch + missing_our_side + missing_their_side` across the filtered set). Shares `_recon_list_filters` (`vendor_id` / `status`) with the list so the page's `openCount` / `totalDiscrepancies` KPIs can't describe only the loaded page. Declared **before** `/{recon_id}` so the literal path wins |
