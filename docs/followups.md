...
- [ ] **`DEPENDABOT_LOCKFILE_PAT` is not configured, so no lockfile ever syncs
      automatically.** `.github/workflows/dependabot-lockfile.yml` regenerates
      the frontend pnpm lockfile and (as of this change) the backend
      `requirements{,-dev}.lock`, then pushes the result back onto the
      Dependabot branch. Its `push` job gates on that PAT and skips with a log
      line when it is unset — and
      `gh api repos/:owner/:repo/dependabot/secrets` currently reports
      `total_count: 0`, so **the gate has never opened**. That is why every
      Dependabot pip PR arrives red: the regeneration half works, the push half
      silently no-ops, and the stale lock fails
      `backend/tests/test_dependency_lock_sync.py`. Until the PAT exists, each
      Dependabot manifest bump needs the locks regenerated by hand (commands in
      [backend/CLAUDE.md](../backend/CLAUDE.md) § Dependency lock).
      **Durable fix:** create a fine-grained PAT scoped to this repo with
      `Contents: Write`, then store it in the **Dependabot** secret store (NOT
      the Actions store — Dependabot-authored PRs cannot read Actions secrets):
      `gh secret set DEPENDABOT_LOCKFILE_PAT --app dependabot`. The workflow
      header documents the full rationale.
      **Trigger:** next Dependabot PR that lands red on a lockfile mismatch.
