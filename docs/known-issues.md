...
## A European-convention statement amount reconciles at 100× its real value

**Diagnosed 2026-08-16** while closing the statement reader's skipped-row
follow-up. Pre-existing; not introduced by that change.

`vendor_statement_recon._money` normalizes a raw amount string by stripping the
...
That is correct under the American convention and silently wrong under the
European one, where the comma is the *decimal* separator. A supplier statement
listing `850,00` (€850.00) is read as **`85000`** — a hundredfold overstatement
of an open item.

It is not caught upstream, because the reader agrees it is money.
`extraction_adapters/statement_extraction._AMOUNT_TOKEN`
(`^\(?[-+]?[$€£]?\d[\d,]*(?:\.\d{1,2})?\)?$`) matches `850,00` — the `[\d,]*`
run absorbs `50,00` — and `_is_money` then treats the comma as the
thousands-separator evidence that distinguishes money from a bare terms/aging
integer. So the token is confidently classified, passed through as the string
`"850,00"`, and mis-parsed downstream.

Both intake paths reach it: the PDF/text reader and `parse_statement_csv`, which
share `_money`.

**Blast radius, and why it is not silent-catastrophic.** Reconciliation compares
the supplier's figure against our own ledger, so an inflated line surfaces as an
`amount_mismatch` recon line rather than quietly reconciling — it fails loud in
the place a clerk looks. The cost is a persisted wrong figure, a vendor pushed
over the `close-readiness` materiality threshold on a phantom balance, and a
reviewer sent to chase a discrepancy that does not exist. It moves no money: no
payment path reads a recon line.

The adjacent case fails safe by accident — `1.200,00` matches nothing, so the
row is skipped rather than mis-booked. Only the no-thousands-separator form
(`NNN,NN`) is mis-read.

**Recommended fix.** Not "detect the locale from the token": `1,200` is 1200
American and 1.2 European, and no per-token rule resolves that. Two durable
shapes, in preference order:

1. **Decide the convention per document, then parse every token under it.** A
   statement is internally consistent, so the whole token set disambiguates
   what one token cannot (any token with both separators fixes the convention;
   failing that, a `,\d{2}$` token with no period anywhere is European). Parse
   under the resolved convention and record it in `meta.extraction` alongside
   the existing provenance, so the reader states its reading rather than
   implying one.
2. **Refuse the ambiguous form**, consistent with the reader's documented
   "skip rather than guess" posture — treat a bare `NNN,NN` as ambiguous, count
   it in the new `ambiguous` skip class the reader already reports, and let the
   clerk supply a CSV. Cheaper and strictly safe, but it makes every European
   statement unreadable, which is a product regression rather than a fix.

Either way the change belongs with pure-function tests over both conventions,
and it alters acceptance behaviour for every statement — which is why it was not
folded into the round that diagnosed it.

Refs: `backend/app/services/vendor_statement_recon.py::_money`,
`backend/app/services/extraction_adapters/statement_extraction.py::_is_money`,
[vendor-statement-reconciliation.md](../backend/docs/vendor-statement-reconciliation.md).
