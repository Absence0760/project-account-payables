...
- [x] Adverse media screening — `ScreeningResult.categories` (`("adverse_media",)`) on all four adapters, folded through `services/sanctions_categories` into the compliance verdict reasons, the persisted `sanctions_checks` row, the `vendor.screened` audit row, the vendor risk sub-score (floored above a bare `review_required`), the screening-trail + review-queue API, and a "Negative news" pill on `/vendors/screening`. A `clear` verdict carrying negative news becomes a `hold` (fail-closed) — see [decisions.md](decisions.md) §34.
...
- [x] **Stylesheet-wide colour-token/contrast guard** — `frontend/src/lib/a11y/tokenPairing.test.ts` (vitest, no browser) scans every `.css`/`<style>` in `src/` for a same-rule pair below SC 1.4.3, a `var(--token, fallback)` contradicting its token, and a `var()` on a token nothing assigns. Added because a contrast failure recurs per *surface*, not per route, so the axe route list kept catching one instance and missing its twin. Closing it introduced `--success{,-strong}` / `--danger{,-strong}` beside `--accent{,-strong}` and fixed 99 findings; `/billing`, `/reports`, `/experiments`, `/cfo` joined the axe list in the same pass. Runtime brand overrides get an inline contrast advisory on `/organization` + `/admin/partner`. A follow-up pass extended it to a rule's own `opacity` — which composites text toward the backdrop, invisibly to both earlier checks — after `/cfo` joining the axe list caught `.kpi-sub` at 4.24:1; all 11 instances were a redundant fade over already-muted text. Translucent-tint badges are the adjacent class, measured but deferred (`docs/followups.md`). See `docs/decisions.md` §28
...
**Status:** In progress (per-tenant brand config + frontend CSS-var theming shipped; branded outbound PDFs + emails shipped; custom-domain tenant resolution shipped — vanity-host → tenant mapping on `settings.brand.custom_domains`, JWT cross-check preserved; custom-domain admin UI + endpoint pair shipped — `GET/PUT /api/organization/branding/custom-domains`, admin-mutate, normalized + cross-org-unique + audited, with the `/organization` Custom Domains panel; **partner/reseller admin shipped** — `Organization.parent_org_id` (migration 0065) + admin-gated `/api/partner` list/read/write child branding, SQL-layer-scoped to the caller's children, + the `/admin/partner` panel; **link provisioning shipped** — two-sided-consent attach (child mints an HMAC-signed single-use link code; partner redeems it) + scoped detach, `FEOH_PARTNER_LINK_SIGNING_KEY` fail-closed; **new-tenant provisioning shipped** — `POST /api/partner/children/provision` wraps `provision_tenant` + stamps `parent_org_id` + audits both trails, with the `/admin/partner` "Create child tenant" modal; the TLS/DNS provisioning runbook now ships; **the SPA now resolves a vanity host end to end** — `PUBLIC_PLATFORM_DOMAINS` + `hostRouting.ts` classify the hostname, a vanity host sends no `X-Tenant-Slug` and calls `/api` same-origin so the backend's `Host` fallback resolves it, and both layouts gate on `hasTenantContext()` rather than the slug; **per-tenant outbound links shipped** — `settings.brand.tenant_url_template` + one `utils/tenant_urls` resolver across all ten call sites; **per-tenant passkeys shipped** — RP ID + origins resolved from the tenant's own registered custom domains, fail-closed against a forged `Host`, with `webauthn_credentials.rp_id` (migration 0091) making cross-host credentials legible. What remains is SSO, see Open below)
**Open:** SSO on a vanity host. The SPA, the outbound links and the passkey relying party are all per-tenant now; what still pins a vanity-domain tenant to the platform hostname is the OIDC `redirect_uri` and the SAML bridge URL in `services/sso.py`. Those two are deliberately still global: they are values **registered at the customer's IdP**, so re-pointing them silently breaks every SSO login until the operator re-registers — an operator-sequenced migration rather than a config read ([decisions.md](decisions.md) §91). Durable fix: a per-org callback override plus the IdP re-registration step in the runbook. *(c) sized-but-unstarted.* Tracked in [followups.md](followups.md).
