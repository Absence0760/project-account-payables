...
- [x] **Stylesheet-wide colour-token/contrast guard** — `frontend/src/lib/a11y/tokenPairing.test.ts` (vitest, no browser) scans every `.css`/`<style>` in `src/` for a same-rule pair below SC 1.4.3, a `var(--token, fallback)` contradicting its token, and a `var()` on a token nothing assigns. Added because a contrast failure recurs per *surface*, not per route, so the axe route list kept catching one instance and missing its twin. Closing it introduced `--success{,-strong}` / `--danger{,-strong}` beside `--accent{,-strong}` and fixed 99 findings; `/billing`, `/reports`, `/experiments`, `/cfo` joined the axe list in the same pass. Runtime brand overrides get an inline contrast advisory on `/organization` + `/admin/partner`. See `docs/decisions.md` §28
