...
- [ ] Developer docs + sandbox keys against the local-first stack; key-management UI in org settings — developer docs ship (backend/docs/public-api.md, incl. the published OpenAPI/Swagger surface); keys are minted per-org and work against the local-first stack today (mock billing adapter grants `public_api`). **Still open:** the frontend key-management + usage UI in org settings (frontend track — the read API `GET /api/api-keys` + `GET /api/api-keys/{id}/usage` it will call is stable and admin-gated)
