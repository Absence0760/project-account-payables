...
**Status:** In progress — **web runtime + full starter locale set shipped**: `frontend/src/lib/i18n/` (locale negotiation, typed `en` catalogue + the full `de/fr/es/pt-BR/ja` set as lazy chunks, lazy loader registry, reactive `m()`/`setLocale()`/`initLocale()`, ICU plurals, `<html lang/dir>`, locale picker with endonyms, `messages_parity` vitest) with the shell/nav + **dashboard** + **invoices list** + **payments** + **vendors** + **exceptions** + **notifications** + **contracts** + **recurring** + **organization settings** + **cfo/analytics** + **expenses** (incl. the **`ExpenseModal`** + **`PolicyModal`** dialogs) + the **procurement routes** (**requisitions** + **intake** + **catalogs** + **budgets** + **purchase-orders** + **goods-receipts**) + **positive-pay** + the **supplier portal** (every `routes/portal/**` page) + the **authentication & onboarding routes** (the pre-auth `login` + `login/mfa` + `login/{sso,saml}-callback` + `signup` + `change-password` + `verify` pages, under the `auth.*` namespace) extracted; `formatMoney` **and per-row dates** follow the active locale (the shared `utils/time.ts::formatDate` is locale-aware and now drives the dashboard / payments / exceptions row dates that previously hardcoded `en-US`). The mobile ARB track has likewise extended to **approvals** + **capture** + **advanced-search** + **cash-flow** + **contracts (list + detail)** + **exception detail** on top of its prior screens. Remaining: extract the remaining web routes (the procurement create/edit modals) + the rest of the mobile screens (invoice detail/edit, payment queue/runs, login, admin/org settings); server-side email localization is **shipped**. See `frontend/CLAUDE.md` → i18n.
