...
| `/adaptive` | Approval-pattern learning, baseline anomalies, advisory workflow suggestions (read-only + dismiss); smart-routing + auto-approve-threshold recommend/apply; **feedback loop** (`GET /adaptive/feedback` — reads the realised human OUTCOMES of auto-approved invoices from `audit_log` (voids/re-rejections/corrections), folds the overturn rate back into the threshold recommendation so it pulls BACK when overturns climb, and surfaces a real `auto_approval_overturn_rate` + `recommendation_acceptance_rate` effectiveness signal with an honest insufficient-data state; deterministic/no-LLM, read-only, compute-on-read, PII-free `adaptive_feedback.viewed` access audit) |
