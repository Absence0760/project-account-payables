...
| `/cash-flow` | **AI Cash-Flow Copilot (Phase 1 — read-only)** — thin façade over the assistant orchestrator giving finance leaders a NL cash-planning surface: `POST /copilot` + `POST /copilot/stream` (SSE, same `tool`/`delta`/`done`/`error` contract as `/assistant`), gated `admin/ap_manager/cfo` (NOT `ap_clerk`), entity-scoped. Registers four read-only planning tools alongside the assistant's (`get_cashflow_forecast`, `get_cash_position`, `run_payment_whatif`, `optimize_discount_capture`) — every money figure comes from existing deterministic pure functions (`services/analytics` `bucket_outflows`/`compute_cash_position`/`apply_payment_timing_scenario`, `services/discount_optimizer.optimize`), serialized as EXACT decimal strings (never float); `run_tool` enforces the per-tool role gate (a clerk gets a clean refusal, never data/500). No enact routes yet (Phases 2–3 planned). Gated by `FEOH_CASHFLOW_COPILOT_ENABLED`. See `docs/cash-flow-copilot.md` |
