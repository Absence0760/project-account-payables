...
| `/approvals/teams` | **Microsoft Teams interactive approval** — `POST /approvals/teams/interactivity` (PUBLIC, no JWT). Approve/reject an assigned invoice from a Teams approval card, no app login. Gated by the Teams Outgoing-Webhook HMAC (`Authorization: HMAC <base64(hmac-sha256 raw_body)>` with `FEOH_TEAMS_SECURITY_TOKEN`, optional `X-Teams-Request-Timestamp` ±5-min replay window) **and** the signed single-use action token in the card action (the email-approval `email_action_token` reused, bound to a `teams` channel + the intended approver); the decision runs the normal `services/review` path (segregation + CFO gate + thresholds + immutable audit row + approval signature). Opaque 200 ack on every path — never enumerates. `teams_approvals.py`; see `backend/docs/teams-approval.md` |
...
| `payment_methods.py` | Single source of truth for what a `Payment.method` rail means to regulatory reporting — which rails the payer files on a 1099-NEC/MISC vs which the card settlement entity files on a 1099-K (`virtual_card` is the only card rail; unknown/`NULL` stays reportable). Pure helpers + a SQL predicate consumed by `tax_1099.build_1099_report`. `tests/test_payment_methods.py` is the drift guard — a rail added to the `PaymentMethod` enum, any adapter's `supported_methods`, or the corridor's `CORRIDOR_OVERRIDE_FEES` fails the suite until it is classified. See `backend/docs/tax-1099.md` § Card payments are excluded |
...
**Control plane**: Organization, User, Role, UserRole, ExtractionUsage, ApiKey, ApiKeyUsage, Plan, Subscription, WebhookSubscription, WebhookDelivery, WebAuthnCredential
**Tenant-scoped**: Entity, Invoice, InvoiceLineItem, InvoiceExtractionResult, Vendor, VendorChangeRequest, PurchaseOrder, POLineItem, GoodsReceipt, GRLineItem, QualityInspection, GLAccount, PaymentRun, PaymentSchedule, Payment, VirtualCard, CardRebate, WorkflowDefinition, WorkflowInstance, WorkflowStep, AuditLog, Exception, AgentDecision, Notification, Contract, ContractLineItem, SupplierChatThread, SupplierChatMessage, ExpenseReport, Expense, ExpensePolicy, CorporateCardTransaction, ExpensePreapproval, DiscountOffer, RecurringInvoiceTemplate, VendorStatementReconciliation, VendorStatementReconLine, WorkflowExperiment
...
| `FEOH_TEAMS_SECURITY_TOKEN` | (empty) | Microsoft Teams Outgoing-Webhook **security token** verifying the interactive-card POST to `/api/approvals/teams/interactivity` (Teams sends `Authorization: HMAC <base64(hmac-sha256 raw_body)>` keyed by the base64-decoded token). Empty → Teams interactive approval is OFF: every inbound POST rejected (fail-closed, no hardcoded fallback). The card's signed action token reuses `FEOH_EMAIL_ACTION_SIGNING_KEY` (bound to a `teams` channel). NON-secret base64 dev value committed in `.env.development`; real token via sops. See `backend/docs/teams-approval.md`. |
