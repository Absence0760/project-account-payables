...
| `/payments` | Payment listing, payment runs (create/execute). Run creation refuses any invoice carrying an unresolved (`open`/`escalated`) exception in `PAYMENT_BLOCKING_EXCEPTION_TYPES` — `duplicate`, `fraud_flag`, `line_total_mismatch` — since approval gates on none of them; resolving/dismissing it is the human sign-off. See `backend/docs/payments.md` § Financial-integrity exception gate |
