...
| `/partner` | **Partner / reseller multi-tenant admin** (white-label) — a partner org administers its branded **child** tenants (linked by the control-plane self-FK `Organization.parent_org_id`, migration 0065). `GET /partner` (caller's child tenants + `is_partner` derived from having ≥1 child; standalone org → empty `is_partner:false`), `GET /partner/children/{id}/branding` (read a child's `BrandConfig`), `PUT /partner/children/{id}/branding` (push a child's brand — persists to the CHILD's `settings.brand`, preserves `custom_domains`, audits `organization.branding_updated` `via:partner` into the CHILD's trail, PII-free). Admin-only; JWT-gated via `get_tenant` (org-claim cross-check). **Tenant isolation at the data layer**: every child query is scoped `parent_org_id == caller org id`, so a partner can't reach an org it didn't parent — a non-child id is the same opaque 404. `/admin/partner` panel — see `docs/white-label.md § Partner / reseller admin` |
