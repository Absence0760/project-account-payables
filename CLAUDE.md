...
| Prefix | Purpose |
|--------|---------|
| `/auth` | Login, logout, profile (JWT + Redis blocklist), MFA enroll/verify/disable, MFA challenge; WebAuthn/passkey MFA (`/auth/mfa/passkey/{register,register/verify}`, `GET/DELETE /auth/mfa/passkey[/{id}]`, `/auth/mfa/passkey/authenticate[/verify]` — passkey is a separate code path, gated by `AP_MFA_ENABLED`; authenticate endpoints are challenge-token-gated/public, register/list/delete are JWT). **Changing an existing second factor is a step-up operation**: `POST /auth/mfa/enroll`, `POST /auth/mfa/passkey/register` and `DELETE /auth/mfa/passkey/{id}` take an optional `{password?, code?}` body and REQUIRE one of them (account password via the shared `pwd_context`, or a code from the currently enrolled authenticator) whenever the account already has a live factor — an enabled TOTP secret OR any registered passkey, so removing a factor is gated exactly like adding one; first-time enrollment stays frictionless. Step-ups are throttled 5/min per ACCOUNT and write a PII-free `auth.mfa.step_up.failure` audit row on failure (same helpers cover `/auth/mfa/disable`). Enroll-start also no longer touches the account: the candidate TOTP secret waits in Redis (`mfa:pending_enroll:<user_id>`, `AP_MFA_ENROLL_PENDING_TTL_SECONDS`) and `/auth/mfa/enroll/verify` is the only writer of `mfa_secret`/`mfa_enabled`, so an abandoned enrollment can never leave the account single-factor. See `docs/authentication.md` § Per-user enrollment |
