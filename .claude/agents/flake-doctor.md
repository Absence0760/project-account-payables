...
- **Invoice extraction** — queued to an in-process pool of 3 worker threads draining a queue (`AP_EXTRACTION_MODE=local`). The upload returns before extraction finishes; the extracted fields / line items appear later.
