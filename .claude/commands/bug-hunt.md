...
- **Respect tenancy & secrets.** Never bypass tenant isolation (`get_tenant` / `get_tenant_db` is the chokepoint — never hardcode an `ap_<slug>` name, never build a tenant engine outside `get_tenant_db`, never run tenant-data queries against the `account_payables` control plane), never log PII/banking data, never leak secrets. Treat auth/tenancy/migrations/approval gates/money/audit-trail/PII as load-bearing (mandatory review pass, step 7). (Rail 11.)
