...
2. **Diff that list against the register's sections.** Every provider needs a row; every row needs a provider. **Known drift to confirm or close on each run** — these adapter families have no section in the register today: `billing_adapters` (Stripe Billing — processes the *customer's own* billing data), `chat_notification_adapters` (Slack / Microsoft Teams incoming webhooks), `enrichment_adapters` (Dun & Bradstreet / Clearbit — vendor identifiers out), `qms_adapters` (the `generic` HTTP skeleton). Report each until it is either added or explicitly ruled out in the doc.
