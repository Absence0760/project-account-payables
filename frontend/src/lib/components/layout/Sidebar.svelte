<script lang="ts">
	...
	// Role + granular-permission checks bound to the auth store; reads
	// reactive state so the visible-entry list recomputes if either changes.
	const has = (...roles: string[]) => auth.hasAnyRole(...roles);
	const canPerm = (perm: string) => auth.can(perm);
	...
	let entries = $derived(NAV.filter((e) => isEntryVisible(e, has, canPerm)));
	...
	function hrefFor(entry: NavEntry): string {
		return entry.kind === 'link' ? entry.href : (groupHref(entry, has, canPerm) ?? '#');
	}
</script>
