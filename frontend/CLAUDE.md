...
- `isPositiveAmount(amount)` is the **predicate** companion — "is this string-Decimal worth rendering at all?" (zero / null / unparseable → `false`). Use it to gate an optional money element (the `/tax` card-excluded column + KPI sub-line). It is deliberately NOT money arithmetic: never add, subtract or compare two amounts client-side — render both figures and let the backend own any delta. Covered by `src/lib/utils/money.test.ts`.
